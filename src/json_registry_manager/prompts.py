...
def text(message: str, default: str = "") -> str | None:
    if _q:
        return _q.text(message, default=default, style=JRM_STYLE).ask()
...
def confirm(message: str, default: bool = False) -> bool | None:
    if _q:
        return _q.confirm(message, default=default, style=JRM_STYLE).ask()
...
def select(message: str, choices: list[Choice], default: Any = None) -> Any:
    if _q:
        ...
        return _q.select(message, choices=qchoices, default=default, style=JRM_STYLE).ask()
    if message:
        print(message)
...
def autocomplete_select(message: str, choices: list[Choice]) -> Any:
    ...
    if _q:
        ...
        answer = _q.autocomplete(
            message, choices=list(mapping), match_middle=True, ignore_case=True, style=JRM_STYLE
        ).ask()
...
def press_any_key_to_continue(message: str = "Press Enter to continue...") -> None:
    if _q:
        _q.press_any_key_to_continue(message, style=JRM_STYLE).ask()
    else:
        input(message)
